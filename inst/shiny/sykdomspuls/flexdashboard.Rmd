...
GLOBAL$weeklyTypes <- GLOBAL$dailyTypes <- CONFIG$SYNDROMES[CONFIG$SYNDROMES %in% CONFIG$SYNDROMES[websiteInternal == TRUE]$tag]
GLOBAL$weeklyAges <- GLOBAL$dailyAges <- CONFIG_OLD$AGES
GLOBAL$dateMax <- pool %>% tbl("spuls_standard_results") %>% summarize(max(date)) %>% collect() [[1]]
GLOBAL$dateMinRestrictedRecent <- GLOBAL$dateMax - 365
GLOBAL$dateMinRestrictedLine <- GLOBAL$dateMax - 365 * 15
GLOBAL$dailyCounties <-  pool %>% tbl("spuls_standard_results") %>%
  filter(granularity_geo == "county", granularity_time == "daily")  %>%
  summarize(distinct(location))%>% collect()
names(GLOBAL$dailyCounties) <- GetLocationName(GLOBAL$dailyCounties)
GLOBAL$weeklyCounties <-  pool %>% tbl("spuls_standard_results") %>%
  filter(granularity_geo == "county", granularity_time == "weekly")  %>%
  summarize(distinct(location))%>% collect()
names(GLOBAL$weeklyCounties) <- GetLocationName(GLOBAL$weeklyCounties)
GLOBAL$weeklyWkyr <- pool %>% tbl("spuls_standard_results") %>%
  summarize(distinct(weekyr)) %>% collect()
GLOBAL$outbreakswkyr = GLOBAL$weeklyWkyr
...
suppressWarnings(pd[, bottom := 0])
  cat(file=stderr(), typeof(pd[["displayDay"]]))
...
weeklyBarometerPlotData <- reactive({
  if (is.null(input$weeklyBarometerCounty)) {
    ...
    retData <- pool %>% tbl(x_table) %>%
        filter(tag == x_tag &
               granularity_time == "weekly" &
               (granularity_geo == "county" | granularity_geo == "country") &
               age==x_age) %>% collect()
