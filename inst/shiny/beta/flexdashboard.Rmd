...
table <- pool %>% tbl("spuls_standard_results")
completeness <- sykdomspuls::calculate_completeness("Norge", year(GLOBAL$dateMax), table=table)

q <- ggplot(tail(completeness, 8)) + geom_col(aes(x=week, y=completeness), fill=fhiplot::base_color) +
 ggtitle("Kompletthet siste 8 uker") +
  fhiplot::theme_fhi_lines() +
  xlab("Uke ") +
  ylab("Kompletthet (%)") +
  ylim(0, 103) 
output$kompletthet_plot <- renderPlot(q)
output$kompletthet_plot2 <- renderPlot(q)
output$kompletthet_plot3 <- renderPlot(q)
output$kompletthet_plot4 <- renderPlot(q)
...
pd <- pool %>% tbl("spuls_standard_results") %>%
    filter(tag == "respiratoryinternal" & age=="15-19" & location_code=="Norge" &
...
output$storyboard1 <- renderPlot({
  q <- fhiplot::make_line_threshold_plot(pd,x="date",dataVal="n",L1="bottom",L2="threshold2",L3="threshold4",L4="top",allPoints = FALSE, title="Øvre-luftvei diagnose i 15-19 åringer i Norge", pointShift = -3.5, weekNumbers=TRUE, step=FALSE, GetCols=GetCols, legend_position = "bottom")
...
output$weeklyNumberPlotBrush <- renderPlot({
  ...
  output$weeklyNumberPlot <- renderPlot({
    pd <- calculate_confidence_interval(weeklyPlotData(), last_weeks=8)
...
influensa_data <- reactive({
  ...
  data <- calculate_confidence_interval(data)
  return(data)
  })
  
output$influensa_plot <- renderPlot({
  data <- influensa_data()
  cat(file=stderr(), data$low_p*100)
  cat(file=stderr(), data$high_p*100)
  t2 <- Getlocation_name(input$influensaCounty)
  title<- paste0("Prosent pasienter med influensa i ", t2)
  q <- fhiplot::make_influenza_threshold_chart(data, title, weeks=c(30,29))
  return(q)
...
completeness_data <- reactive({
  x_location <- input$completenessCounty

  table <- pool %>% tbl("spuls_standard_results")
  return(sykdomspuls::calculate_completeness(x_location, year(GLOBAL$dateMax), table=table))
  }) 
  
output$completeness_plot <- renderPlot({
  data <- completeness_data()
  t2 <- Getlocation_name(input$completenessCounty)
  q <- ggplot(data) + geom_col(aes(x=week, y=completeness), fill=fhiplot::base_color) +
    ggtitle(glue::glue("Estimert data kompletthet i {t2} for dette året")) +
    fhiplot::theme_fhi_lines() +
    xlab("Uke ") +
    ylab("Kompletthet (%)") +
    ylim(0, 103)
...
Grafen viser estimert data kompletthet per uke. 
...
selectInput("completenessCounty", "Fylke", as.list(GLOBAL$weeklyCounties), selected = GLOBAL$weeklyCounties[1])
