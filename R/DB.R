...
SaveShinyAppDataToDB <- function(){
  ...
  CONFIG_OLD <- readRDS("/data_app/sykdomspuls/config.RDS")
  ...
  resYearLine <- readRDS("/data_app/sykdomspuls/resYearLine.RDS")[type %in% GLOBAL$weeklyTypes]
  ...
  dplyr::copy_to(dest=db,
                 df=resRecentLine,
                 name="resRecentLine",
                 temporary=FALSE,
                 overwrite=TRUE
  )
  RMariaDB::dbExecute(db,
