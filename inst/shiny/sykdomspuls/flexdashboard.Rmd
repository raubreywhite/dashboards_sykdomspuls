...
weeklySignalData <- reactive({
  ...
  #)
  return(outbreaks$df[outbreaks$df$tag!="influensa" & outbreaks$df$wkyr==input$weeklyOutbreakWeek,])
})
...
renderTable({
  data <- outbreaks$df[outbreaks$df$tag!="influensa" & outbreaks$df$wkyr==input$weeklyOutbreakWeek,]
...
#tableOutput("weeklySignalTableDF")
renderTable({
  data <- outbreaks$dk[outbreaks$dk$tag!="influensa" & outbreaks$dk$wkyr==input$weeklyOutbreakWeek,]
  if(input$weeklyOutbreakHideEmpty){
    data <- data[data$High!="",]
  }
  if(input$weeklyOutbreakSort=="zscore"){
